...
import {
  parseGitTransferProgressLine,
  summarizeGitTransferProgress,
} from '../gitTransferProgress';

describe('gitTransferProgress', () => {
  it('parses receiving progress with amount and speed', () => {
    const parsed = parseGitTransferProgressLine(
      'Receiving objects: 96% (32866/34235), 322.40 MiB | 8.82 MiB/s',
    );
  ...
  it('summarizes receiving as done when resolving is active', () => {
    ...
    expect(summary.phases.find((phase) => phase.key === 'resolving')).toEqual(expect.objectContaining({
      state: 'active',
      percent: 2,
    }));
  });

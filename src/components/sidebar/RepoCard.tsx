...
export const RepoCardHeader: React.FC<RepoCardHeaderProps> = ({ title, collapsed, onToggleCollapsed, toggleTitle, actions }) => (
...
export const RepoCardContent: React.FC<{ children: React.ReactNode; className?: string; style?: React.CSSProperties }> = ({ children, className, style }) => (
  <div className={`repo-card-content${className ? ` ${className}` : ''}`} style={style}>
    {children}
  </div>
);

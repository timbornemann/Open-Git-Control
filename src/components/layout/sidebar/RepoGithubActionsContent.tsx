...
import { CheckCircle2, Clock3, Copy, ExternalLink, GitBranch, GitPullRequest, Plus, RefreshCw, Search, XCircle } from 'lucide-react';
          ...
          <>
            <RepoCardToolbar>
              {(['open', 'closed', 'all'] as const).map((filter) => (
                <button key={filter} onClick={() => props.setPrFilter(filter)} className={`repo-filter-chip ${props.prFilter === filter ? 'active' : ''}`}>
                  {filter === 'open'
            ...
            {props.showCreatePR && (
              <RepoCardContent className="repo-form-stack" style={{ borderBottom: '1px solid var(--line-subtle)' }}>
                <input
                  ...
                  value={props.newPRTitle}
                  onChange={(e) => props.setNewPRTitle(e.target.value)}
                />
                <textarea
                  className="repo-filter-input"
                  placeholder={t('generated.components.layout.sidebar.githubconnectedcontent.description_optional_30003d39')}
                  value={props.newPRBody}
                  onChange={(e) => props.setNewPRBody(e.target.value)}
                  rows={2}
                ...
                <div className="repo-inline-fields">
                  <input
                    ...
                    value={props.newPRHead}
                    onChange={(e) => props.setNewPRHeadInput(e.target.value)}
                  />
                  <input
                    className="repo-filter-input"
                    type="text"
                    placeholder={t('generated.components.layout.sidebar.githubconnectedcontent.base_branch_e03dac14')}
                    value={props.newPRBase}
                    onChange={(e) => props.setNewPRBase(e.target.value)}
                  />
                </div>
                <div className="sidebar-row-actions">
                  <button className="staging-tool-btn" onClick={() => props.setShowCreatePR(false)}>
                    {t('generated.components.confirm.cancel_035b7526')}
                  </button>
                  <button className="staging-tool-btn" onClick={props.onCreatePR} disabled={!props.newPRTitle.trim()}>
            ...
            <RepoCardContent className="repo-card-scroll repo-scroll-lg">
              {props.prLoading && props.pullRequests.length === 0 && (
                <div className="repo-state-text">{t('generated.components.layout.sidebar.githubconnectedcontent.loading_pull_requests_f64f6445')}</div>
              )}

              {!props.prLoading && props.pullRequests.length === 0 && (
                <div className="repo-state-text">{t('generated.components.layout.sidebar.githubconnectedcontent.no_pull_requests_4e17ae83')}</div>
              )}
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: '0.71rem', color: 'var(--text-secondary)', marginTop: '2px' }}>
                          ...
                          {(() => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {ci?.badge === 'pending' && (
                                  <RefreshCw size={11} className="spin" style={{ marginRight: '4px', verticalAlign: 'text-bottom' }} />
                                )}
                                {badgeStyles.label}
                              </button>
                            );
                      ...
                      {selectedPrNumber === pr.number && props.prCiByNumber[pr.number] && (
                        ...
                        >
                          {(props.prCiByNumber[pr.number]?.workflowRuns || []).slice(0, 5).map((run) => (
                            <div
                              key={run.id}
                              style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '4px 10px', alignItems: 'center', fontSize: '0.72rem' }}
                            >
                              <div style={{ minWidth: 0 }}>
                                ...
                                <div style={{ color: 'var(--text-secondary)' }}>
                                  ...
                                  {run.event} | {formatDuration(run.startedAt, run.updatedAt)}
                                </div>
                              </div>
                              <button
                      ...
                      <div className="sidebar-row-actions">
                        {pr.state === 'open' && (
                          <>
                            <button className="staging-btn-sm" onClick={() => props.onMergePR(pr.number, 'merge')}>
                              {t('generated.components.layout.main.mainprimarypane.merge_83b759bf')}
                            </button>
                            <button className="staging-btn-sm" onClick={() => props.onMergePR(pr.number, 'squash')}>
                              {t('generated.components.layout.sidebar.repogithubactionscontent.squash_52bce1bb')}
                            </button>
                            <button className="staging-btn-sm" onClick={() => props.onMergePR(pr.number, 'rebase')}>
                              {t('generated.components.layout.main.mainprimarypane.rebase_26c8effa')}
                            </button>
                          </>
                        )}
                        <button className="staging-btn-sm" onClick={() => props.onOpenPR(pr.htmlUrl)}>
                          <ExternalLink size={12} />
                        </button>
                        <button className="staging-btn-sm" onClick={() => props.onCopyPRUrl(pr.htmlUrl)}>
                          <Copy size={12} />
                        </button>
                        <button className="staging-btn-sm" onClick={() => props.onCheckoutPR(pr.number, pr.head)}>
                          <GitBranch size={12} />
                        </button>
                      </div>
                    </div>
          ...
          <>
            ...
            <RepoCardContent className="repo-card-scroll repo-scroll-md">
              ...
              {workflowRunsError && workflowRuns.length === 0 && (
                <div className="repo-state-text" style={{ color: 'var(--status-danger)' }}>
                  {workflowRunsError}
                </div>
              )}

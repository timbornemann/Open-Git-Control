...
export const GithubWorkspaceView: React.FC = () => {
  ...
  useEffect(() => { if (!github.isAuthenticated) setSelected(null); }, [github.isAuthenticated]);
  ...
  const openRepository = (repo: GitHubRepositoryDto) => {
    ...
    setWarning(null);
    const paths = localRepos.get(toRepoIdentity(repo.htmlUrl) || '') || [];
    if (!online || !paths.length || paths.some((path) => repository.activeRepo && normalizeRepoPathKey(path) === normalizeRepoPathKey(repository.activeRepo))) return;
    void repository.onSwitchRepo(paths[0]).then((activated) => {
      if (!activated) setWarning(tr('Lokaler Repo-Wechsel abgebrochen. Du kannst die Remote-Ansicht weiter nutzen.', 'Local repository switch cancelled. You can still use the remote view.'));
  ...
  const onCreated = (result: { repository: GitHubRepositoryDto; brandedReadme: boolean; warning?: string }) => {
    ...
    setWarning(result.brandedReadme ? null : result.warning || tr('Repo erstellt. Die README-Vorlage konnte nicht angewendet werden.', 'Repository created. The README template could not be applied.'));
    void catalog.refresh();
  };

  return <div className="github-workspace">
    {!github.isAuthenticated && <div className="github-workspace__login"><GithubAuthContent
      tokenInput={github.tokenInput} setTokenInput={github.setTokenInput} isAuthenticating={github.isAuthenticating}
      authError={github.authError} setAuthError={github.setAuthError} onTokenLogin={github.onTokenLogin}
      oauthConfigured={github.oauthConfigured} deviceFlow={github.deviceFlow} isDeviceFlowRunning={github.isDeviceFlowRunning}
      deviceFlowError={github.deviceFlowError} onStartDeviceFlowLogin={github.onStartDeviceFlowLogin}
      onCancelAuthentication={github.onCancelAuthentication} onCancelDeviceFlow={github.onCancelDeviceFlow}
      isWebFlowRunning={github.isWebFlowRunning} webFlowError={github.webFlowError} onStartWebFlowLogin={github.onStartWebFlowLogin}
      selectedGithubAuthHelpMethod={github.selectedGithubAuthHelpMethod} onSelectGithubAuthHelpMethod={github.onSelectGithubAuthHelpMethod}
    /></div>}
    {(catalog.offline || catalog.error) && catalog.repos.length > 0 && <div className="github-workspace__offline" role="status">
      {tr('Offline-Momentaufnahme', 'Offline snapshot')} · {catalog.savedAt ? new Intl.DateTimeFormat(locale, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(catalog.savedAt)) : '—'}
      {catalog.error ? ` · ${catalog.error}` : ''}
    </div>}
    {catalog.error && catalog.repos.length === 0 && <div className="github-workspace__error" role="alert">{catalog.error}</div>}
    {!selected ? (github.isAuthenticated || catalog.repos.length > 0) && <GithubCatalog
      repos={catalog.repos} localRepos={localRepos} pinnedIds={pinnedIds} online={online} loading={catalog.loading}
      loadedCount={catalog.loadedCount} onOpen={openRepository} onTogglePin={togglePin}
      onClone={(repo) => github.onClone(repo.cloneUrl, repo.name)} onCreate={() => setShowCreate(true)}
    /> : <GithubRepositoryDetail
      repository={selected} localRepos={localRepos} activeRepo={repository.activeRepo} online={online} warning={warning}
      onBack={() => { setSelected(null); setWarning(null); }} onClone={() => github.onClone(selected.cloneUrl, selected.name)}
      onActivateLocal={repository.onSwitchRepo} onOpenLocal={() => ui.setActiveTab('repo')}
      onOpenReleaseCreator={() => { ui.setActiveTab('repo'); github.onOpenReleaseCreator(); }}
    />}

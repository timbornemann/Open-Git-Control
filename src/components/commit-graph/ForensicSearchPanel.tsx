...
export const ForensicSearchPanel: React.FC<ForensicSearchPanelProps> = ({
  ...
  <div
    style={{
      ...
      padding: '8px',
      display: activeSearchPanel === 'forensic' ? 'flex' : 'none',
  ...
  >
    ...
    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter') void runForensicSearch();
        }}
      ...
      {forensicType === 'line' ? (
        <>
          <input
            type="number"
            min={1}
            value={forensicStartLine}
            onChange={(e) => setForensicStartLine(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === 'Enter') void runForensicSearch();
            }}
            placeholder={t('generated.components.commit_graph.forensicsearchpanel.start_line_74f7b73e')}
            style={{
              width: 120,
              border: '1px solid var(--border-color)',
              backgroundColor: 'var(--bg-panel)',
          ...
          <input
            type="number"
            min={1}
            value={forensicEndLine}
            onChange={(e) => setForensicEndLine(e.target.value)}
            onKeyDown={(e) => {
              if (e.key === 'Enter') void runForensicSearch();
            }}
            placeholder={t('generated.components.commit_graph.forensicsearchpanel.end_line_99a9732d')}
            style={{
              width: 120,
              border: '1px solid var(--border-color)',
              backgroundColor: 'var(--bg-panel)',
      ...
      <button
        onClick={() => void runForensicSearch()}
        ...
        {forensicResults.map((node) => (
          ...
          >
            <button
              ...
              style={{ border: 'none', background: 'transparent', color: 'var(--text-primary)', cursor: 'pointer', fontFamily: 'monospace' }}
            >
              {node.commit.abbrevHash}
            </button>
            <span style={{ color: 'var(--text-secondary)', fontSize: '0.76rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
              {node.commit.subject}
            </span>
            <button
              onClick={() =>
                onOpenDiff?.({
                  source: 'commit',
                  path: forensicPath.trim(),
                  commitHash: node.commit.hash,
                  title: `${node.commit.abbrevHash} - ${forensicPath.trim()}`,
                })
              }
              style={{
                border: '1px solid var(--border-color)',
            ...
            >
              {t('generated.components.commit_graph.forensicsearchpanel.diff_d3567fa5')}
            </button>
          </div>
        ))}
      </div>

      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        ...
        {normalizedSearch && (
          <div className="commit-search-meta" style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '0.74rem', color: 'var(--text-secondary)' }}>
            <span>{matchCount} {t('generated.components.commit_graph.commitsearchtoolbar.matches_7020567c')}</span>
            <button className="commit-search-nav" style={{ border: '1px solid var(--border-color)', backgroundColor: 'var(--bg-panel)', color: 'var(--text-primary)', borderRadius: '4px', padding: '3px 8px', cursor: 'pointer', fontSize: '0.72rem' }} onClick={onJumpToPreviousMatch} disabled={matchCount === 0}>{t('generated.components.commit_graph.commitsearchtoolbar.prev_fa5decb3')}</button>
            <button className="commit-search-nav" style={{ border: '1px solid var(--border-color)', backgroundColor: 'var(--bg-panel)', color: 'var(--text-primary)', borderRadius: '4px', padding: '3px 8px', cursor: 'pointer', fontSize: '0.72rem' }} onClick={onJumpToNextMatch} disabled={matchCount === 0}>{t('generated.components.staging_area.conflictresolverpanel.next_b53e1a35')}</button>
          </div>
        )}
      </div>
    )}

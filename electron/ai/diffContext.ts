...
const extractNumstatTargetPath = (rawPath: string): string => {
  ...
  const pathForRenameExpansion = isQuoted ? decodePorcelainPath(rawPath) || rawPath : rawPath;
  ...
  const expandedPath = pathForRenameExpansion.replace(/\{([^{}]*) => ([^{}]*)\}/g, (_match, _oldPart: string, newPart: string) => newPart);
  if (isQuoted) return expandedPath;

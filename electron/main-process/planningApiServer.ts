...
const callAgentTool = async (name: string, args: JsonObject): Promise<unknown> => {
  switch (name) {
    case 'list_tabs':
      return getTabs();
    case 'list_projects':
...
const handleMcpRpc = async (payload: unknown): Promise<unknown> => {
    ...
    switch (method) {
      ...
      case 'tools/call': {
        ...
        const args = params.arguments && typeof params.arguments === 'object'
          ...
          : {};
        const structuredContent = await callAgentTool(toolName, args);
        return jsonRpcResult(id, {
...
const createPlanningApiRequestHandler = (deps: { getAuthToken: () => string }) => async (
  request: http.IncomingMessage,
  ...
  try {
    ...
    if (url.pathname === '/mcp') {
      ...
      const rpcResponse = await handleMcpRpc(payload);
      if (rpcResponse === null) {
        response.writeHead(204);
        response.end();
        return;
...
<main>
  ...
  <p>Alle Daten- und MCP-Endpunkte benoetigen ein pro App-Prozess erzeugtes Token. Sende es als <code>${AUTH_HEADER_NAME}: &lt;TOKEN&gt;</code> oder <code>Authorization: Bearer &lt;TOKEN&gt;</code>. Das aktuelle Token findest du in den Open-Git-Control Einstellungen unter API/MCP.</p>
  <p>Die lokale API stellt ausschliesslich Planungsfunktionen bereit. Git- und GitHub-Operationen werden nicht ueber REST oder MCP exportiert.</p>
